...
void signalHandler(int signum){
 printf("Server interrupted iwth signal (%d). Shutting down\n",signum);
 close(socket_fd);
 exit(signum);
}
...
int main(int argc , char *argv[])
{
	...
	while(total_received != message_length) {
		received_now= recv(client_sock , return_message+total_received*sizeof(char), message_length-total_received , 0);
		if(received_now < 0) {
	...
	while(total_written < message_length) {
        	num_written = send(client_sock, return_message+total_written*sizeof(char) , message_length-total_written,0);
		if (num_written < 0) {
